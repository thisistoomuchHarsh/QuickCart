          ...
          <div className="flex flex-col gap-4 my-8">
            ...
            <div className="flex gap-2">
              {productData.sizes.map((item, index)=>(
                <button onClick={()=>setSize(item)} className={`py-2 px-4 bg-gray-200 cursor-pointer ${item === size ? 'border border-orange-500' : ''}`} key={index}>{item}</button>
              ))}
            </div>
          </div>
          <button onClick={()=>{addToCart(productData._id, size)}} className="bg-black text-white text-sm py-3 px-8 active:bg-gray-700 cursor-pointer">ADD TO CART</button>
